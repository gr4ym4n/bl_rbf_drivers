...
def output_channel_array_index(channel: 'OutputChannel') -> int:
    return channel.get("array_index", 0)


def output_channel_data_path(channel: 'OutputChannel') -> str:
    return channel.get("data_path", "")


def output_channel_is_enabled(channel: 'OutputChannel') -> bool:
    return channel.get("is_enabled", False)


def output_channel_mute_update_handler(channel: 'OutputChannel', _: 'Context') -> None:
    dispatch_event(OutputChannelMuteUpdateEvent(channel, channel.mute))


def output_channel_name(channel: 'OutputChannel') -> str:
    return channel.get("name", "")


def output_channel_name_set(channel: 'OutputChannel', value: str) -> None:
    cache = output_channel_name(channel)
    channel["name"] = value
    dispatch_event(OutputChannelNameChangeEvent(channel, value, cache))


class OutputChannel(Symmetrical, PropertyGroup):
...
class RBFDriverOutputChannels(Searchable[OutputChannel],
                              Collection[OutputChannel],
                              PropertyGroup):
